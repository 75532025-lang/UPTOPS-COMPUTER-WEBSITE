...
/* --------------------------------------------------------------------------
   Small safety helper: every dynamic string on this site is written with
   textContent (never innerHTML), so user-influenced text can never be
   interpreted as markup. This helper exists as a defence-in-depth guard for
   the few spots that build a URL/attribute from data instead of a text node.
   -------------------------------------------------------------------------- */
...
/* --------------------------------------------------------------------------
   Search suggestions (static term list; input is only ever read via
   textContent-driven buttons, never inserted as HTML)
   -------------------------------------------------------------------------- */
...
/* --------------------------------------------------------------------------
   Small chrome: view-all overlay, mobile menu, scroll-to-top
   -------------------------------------------------------------------------- */
...
/* --------------------------------------------------------------------------
   Product catalogue: single rendering + filtering + sorting module.
   Every card on every page is generated from productsData, so the catalog
   is always the source of truth and filters always match what's rendered.
   -------------------------------------------------------------------------- */
...
function populateTypeChips(container, products) {
  ...
  types.forEach((type) => {
    ...
    if (!count) return; // Filter cleanup: never render a chip with zero items
...
function initCatalog() {
  ...
  // Skip containers explicitly marked as static (e.g. the curated home featured grid)
  ...
  const applyFilters = () => {
    ...
    // Normalize plurals (e.g., "laptops" -> "laptop", "desktops" -> "desktop")
    ...
    cards.forEach((card) => {
      ...
      // Check across name, specs, type, brand, and category
  ...
  // Wire the header search box (if present) to filter this grid live.
...
/* --------------------------------------------------------------------------
   Home page: curated "Featured" grid pulled straight from the catalogue,
   so it can never drift from what laptops.html / accessories.html show.
   -------------------------------------------------------------------------- */
